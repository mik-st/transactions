...
This notebook is for training m6.
# m6   age, gender, country,  education level,  education_title , profession, years_experience  ,  working  > salary
...
rf_model <- ranger(
  salary_eur ~ years_experience +    gender + country + profession,
  data = train,
  num.trees = 500,
  importance = 'impurity',
  regression = TRUE
...
# Calculate errors
errors <- rf_model_predictions$predictions   - test$salary_eur
...
linear_model <- lm( salary_eur ~ years_experience +    gender + country + profession, data = train)
...
errors <- test$salary_eur- linear_model_predictions
...
errors <- test$salary_eur - knn_model_predictions
...
```{r}
# Decision tree model
tree_model <- rpart( salary_eur ~ years_experience + gender + country + profession, data = train, method = "anova")
...
errors <- test$salary_eur - tree_model_predictions
...
xgb_model <- train(salary_eur ~ years_experience + gender + country + profession , data = train , method = "xgbTree", 
...
errors <- test$salary_eur - xgb_model_predictions
