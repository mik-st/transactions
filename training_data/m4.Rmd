...
This notebook is for training m3.
 years_experience ~ age, gender, country,  education level,  education_title , profession
#Models compared:
...
df$education_title <- as.factor(df$education_title)
...
```{r}
 levels(test$profession)   ==  c   (levels(test$profession))
```
...
rf_model <- ranger(
  years_experience ~ age + gender + country + profession,
  data = train,
  num.trees = 500,
  importance = 'impurity',
  regression = TRUE
...
# Calculate errors
errors <- rf_model_predictions$predictions   - test$years_experience
...
input_data= data.frame(age=as.integer(age), 
  ...
  pred <- predict(m4,   data=input_data)
```
...
linear_model <- lm(years_experience ~ age + gender + country + profession, data = train)
...
errors <- df$years_experience - linear_model_predictions
...
knn_model <- train(years_experience ~ age + gender + country + profession, data = train , method = "knn", 
...
errors <- test$years_experience - knn_model_predictions
...
```{r}
# Decision tree model
tree_model <- rpart(years_experience ~ age + gender + country + profession, data = train, method = "anova")
...
errors <- test$years_experience - tree_model_predictions
...
xgb_model <- train(years_experience ~ age + gender + country + profession , data = train , method = "xgbTree", 
                    trControl = trainControl(classProbs = TRUE),
...
errors <- test$years_experience - xgb_model_predictions
